...
const BirdResult = ({ result, type }: BirdResultProps) => {
  ...
  return (
    <Card className="p-6 bg-white/90 backdrop-blur-sm border-forest-medium/20">
      <div className="space-y-6">
        {/* Header */}
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <Bird className="w-6 h-6 text-forest-dark" />
        ...
        {type === 'audio' && result.reasoning && (
          <div className="bg-sage/20 p-4 rounded-lg">
            <div className="flex items-start gap-2">
              <Sparkles className="w-5 h-5 text-forest-medium mt-0.5" />
              <div>
                <h4 className="font-semibold text-forest-dark mb-1">
                  Why this identification?
                </h4>
                <p className="text-sm text-forest-medium">{result.reasoning}</p>
              </div>
            </div>
          </div>
        )}
